...
interface AmenityBreweriesMapClientProps {
  breweries: any[];
  amenityName: string;
  amenityDefinition?: string;
}

export default function AmenityBreweriesMapClient({ breweries, amenityName, amenityDefinition }: AmenityBreweriesMapClientProps) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[650px_1fr] gap-4 max-w-[1700px] mx-auto">
        ...
        <div className="p-4 border-b border-gray-200 bg-gray-50 flex-shrink-0" style={{ fontFamily: "'Source Sans 3', sans-serif" }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-gray-900">{amenityName} Breweries</h2>
            {hasActiveFilters && (
              <button
          ...
          <div className="mt-3 text-sm text-gray-600">
            Showing {startIndex + 1}-{Math.min(endIndex, filtered.length)} of {filtered.length} breweries with {amenityName.toLowerCase()} in Maryland
        ...
        <div className="flex flex-col space-y-4">
          ...
          {/* Amenity Definition Section */}
          {amenityDefinition && (
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4">About {amenityName} Breweries</h2>
              <div 
                className="prose prose-lg text-[#6B6B6B]"
                style={{ fontFamily: "'Source Sans 3', sans-serif" }}
              >
                <p>{amenityDefinition}</p>
              </div>
            </div>
          )}

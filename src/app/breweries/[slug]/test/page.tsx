...
import SimpleBreweryPageTemplateV2 from '@/components/templates/SimpleBreweryPageTemplateV2';
...
// Note: This is a TEST route for iterating on the enhanced brewery page
// Access at: /breweries/[slug]/test
// Production route remains unchanged at: /breweries/[slug]
...
export async function generateMetadata({ params }: BreweryPageProps) {
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: 'website',
      url: `https://www.marylandbrewery.com/breweries/${slug}/test`,
...
export default async function BreweryPageTest({ params }: BreweryPageProps) {
  const { slug } = await params;
  const processed = await getProcessedBreweryData();
  const brewery = processed.breweries.find(
    (b) => (b as any).slug === slug || b.id === slug
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[Brewery Test Page Debug]', {
      ...
      nextOpenDay,
      nextOpenTime,
      todayHours: breweryHours?.[currentDay],
      allHours: breweryHours,
    });
  }

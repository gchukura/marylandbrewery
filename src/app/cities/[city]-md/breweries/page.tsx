...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical: `/cities/${citySlugFromParam}/breweries` },
    openGraph: {
      title,
      description,
      url: `https://www.marylandbrewery.com/cities/${citySlugFromParam}/breweries`,
...
export default async function CityBreweriesPage({ params }: { params: Promise<{ city: string }> }) {
  ...
  const neighborhoods = allNeighborhoods.filter(
    (neighborhood: any) => neighborhood.name.toLowerCase().trim() !== cityName.toLowerCase().trim()
  );
